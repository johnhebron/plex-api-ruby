...
module OpenapiClient
  class GetGlobalHubs200ResponseMediaContainerHubInnerMetadataInner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::GetGlobalHubs200ResponseMediaContainerHubInnerMetadataInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::GetGlobalHubs200ResponseMediaContainerHubInnerMetadataInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'smart')
        self.smart = attributes[:'smart']
      end
    ...
    def hash
      [rating_key, key, guid, type, title, title_sort, summary, smart, playlist_type, composite, icon, view_count, last_viewed_at, duration, leaf_count, added_at, updated_at].hash
    end
