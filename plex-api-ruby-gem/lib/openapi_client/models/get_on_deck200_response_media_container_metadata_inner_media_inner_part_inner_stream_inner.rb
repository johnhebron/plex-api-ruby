...
module OpenapiClient
  class GetOnDeck200ResponseMediaContainerMetadataInnerMediaInnerPartInnerStreamInner
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::GetOnDeck200ResponseMediaContainerMetadataInnerMediaInnerPartInnerStreamInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::GetOnDeck200ResponseMediaContainerMetadataInnerMediaInnerPartInnerStreamInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, stream_type, default, codec, index, bitrate, language, language_tag, language_code, bit_depth, chroma_location, chroma_subsampling, coded_height, coded_width, color_range, frame_rate, height, level, profile, ref_frames, width, display_title, extended_display_title].hash
    end
