...
module OpenapiClient
  class GetLibraryHubs200ResponseMediaContainerHubInnerMetadataInnerMediaInner
    attr_accessor :id
    ...
    def self.openapi_types
      {
        ...
        :'part' => :'Array<GetLibraryHubs200ResponseMediaContainerHubInnerMetadataInnerMediaInnerPartInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::GetLibraryHubs200ResponseMediaContainerHubInnerMetadataInnerMediaInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::GetLibraryHubs200ResponseMediaContainerHubInnerMetadataInnerMediaInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, duration, bitrate, width, height, aspect_ratio, audio_channels, audio_codec, video_codec, video_resolution, container, video_frame_rate, optimized_for_streaming, audio_profile, has64bit_offsets, video_profile, part].hash
    end
