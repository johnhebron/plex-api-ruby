...
module OpenapiClient
  class GetSessionHistory200ResponseMediaContainerMetadataInner
    attr_accessor :history_key
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::GetSessionHistory200ResponseMediaContainerMetadataInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::GetSessionHistory200ResponseMediaContainerMetadataInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'history_key')
        self.history_key = attributes[:'history_key']
      end
    ...
    def hash
      [history_key, key, rating_key, library_section_id, parent_key, grandparent_key, title, grandparent_title, type, thumb, parent_thumb, grandparent_thumb, grandparent_art, index, parent_index, originally_available_at, viewed_at, account_id, device_id].hash
    end
